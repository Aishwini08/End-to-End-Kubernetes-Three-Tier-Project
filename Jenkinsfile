pipeline {
    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            steps {
                // credentialsId matches JCasC definition: "sonarqube-token"
        ...
        stage('ECR Login') {
            steps {
                // FIX: Use AWS_ACCOUNT_ID env var set in previous stage
                // instead of calling aws sts get-caller-identity again
                sh '''
        ...
        stage('Build & Push Backend') {
            steps {
                // FIX: Added prune before backend build too.
                // Original only pruned before frontend — backend could fail
                // with "no space left on device" after frontend filled the disk.
                sh '''
                    docker system prune -af || true
        ...
        stage('Update Helm Chart Tags') {
            steps {
                // FIX: Scoped each sed to its own file.
                // Original ran the same broad `repository:.*` regex across both
                // files in one block — risk of cross-contaminating values.
                sh """
